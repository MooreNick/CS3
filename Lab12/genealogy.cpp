...
public:
    ...
    void setFirstName(std::string newName) { firstName_ = newName; }
...
class NameUpdate : public PersonVisitor {
public:
    void visit(Man* m) override {
        Man* father = static_cast<Man*>(m->getFather());
        if (father != nullptr) {
            std::string newName;
            newName = m->getFirstName() + ' ' + father->getFirstName();
    ...
    void visit(Woman* w) override {
        ...
        if (mother != nullptr) {
            std::string newName;
            newName = w->getFirstName() + ' ' + mother->getFirstName();
...
int main() {
    ...
    std::cout << "After name update:" << std::endl << std::endl;
