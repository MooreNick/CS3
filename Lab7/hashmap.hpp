...
template <typename Key, typename Value,
    ...
    public:
        ...
        std::pair<typename hashmap<Key, Value, Compare, Hash>::Element*, bool> erase(const Key& x);         // erases element with key x, if exists
        Value& operator[] (const Key& x); // returns reference on value of
                                          // element with key, inserts if does not exist
        void rehash(size_t n);

        size_t getBuckets() { return hash_.numBuckets(); };
...
template <typename Key, typename Value, typename Compare, typename Hash>
std::pair<typename hashmap<Key, Value, Compare, Hash>::Element*, bool> hashmap<Key, Value, Compare, Hash>::insert(const Element& x) {

    std::pair<typename hashmap<Key, Value, Compare, Hash>::Element*, bool> retVal;
...
std::pair<typename hashmap<Key, Value, Compare, Hash>::Element*, bool> hashmap<Key, Value, Compare, Hash>::erase(const Key& x) {
    ...
    if (it != elems_[bucket].end()) {    // the element exists, erase it
        ...
        --size_;
        retVal.second = true;
        if (size_ == 0) { // that was the last element
            retVal.first = nullptr;
        }

        else if (it == elems_[bucket].end()) { // still another element but in diff bucket
            auto newBucket = ++bucket;
            
            while (elems_[newBucket].empty()) { // find bucket with element
                ++newBucket;
                if (newBucket == hash_.numBuckets()) { // reached end, means element was before, therefore nullptr is returned
                    retVal.first = nullptr;
                    return retVal;
                }
            }
            it = elems_[newBucket].begin(); // get the address
            retVal.first = &(*it);
        }
        else { // next element in same bucket
            retVal.first = &(*it);
        }
...
Value& hashmap<Key, Value, Compare, Hash>::operator[] (const Key& x) {
    ...
    if (found == nullptr) { // if key not found, create new element with empty value
        auto finder = insert(make_pair(x, Value())); // calling default constructor on Value
        found = finder.first;
        // insert returns a pair, setting found to first part removed second invocation of find()
...
void hashmap<Key, Value, Compare, Hash>::rehash(size_t n) {
    if (n < hash_.numBuckets()) return; // if n is smaller than numBuckets, do nothing

    auto container = std::move(elems_); // storage for elements, elems_ will be reconstructed, therfore it can be destructed

    hash_ = DefaultHash<Key>(n); // use constructor, updates our numBuckets
    elems_.clear(); // clear anything that may be left
    elems_ = vector<list<Element>>(hash_.numBuckets()); // same from hashmap constructor, remaking elems_

    for (auto& e : container) { // put everything back
        for (auto& f : e) {
              insert(f);
        }
    }
